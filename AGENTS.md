...
5. **Calidad**: Si generas código, este debe incluir tests básicos (servicios en backend, validaciones en domain). No se dará por terminada una implementación si existen archivos con errores; todos los problemas deben solucionarse antes de validar la entrega.
6. **Verificación de Flujo**: Antes de dar por finalizada una tarea que afecte a la experiencia de usuario, se debe verificar el flujo completo (navegación, interacciones) mediante el navegador para asegurar que no hay cuellos de botella o elementos no funcionales.
7. **Mentalidad "Auditor"**: Si alguien solicita un cambio a esta arquitectura, debes cuestionarlo y pedir que sea argumentado como si estuviéramos ante un auditor.
